...
test("schedule cursors accept only real Sunday dates", () => {
  ...
  assert.equal(safeSundayCursor("2024-02-29"), undefined);
  assert.equal(safeSundayCursor("2024-02-30"), undefined);
  assert.equal(safeSundayCursor("2024-02-25"), "2024-02-25");
  assert.equal(safeSundayCursor("2023-02-29"), undefined);
});
...
test("schedule display state combines current Sunday emphasis with non-special fill", () => {
  assert.deepEqual(
    getSundayScheduleDisplayState("ward_conference", "2025-02-02", "2025-02-02"),
    { isCurrent: true, hasSubtleFill: true },
  );
  assert.deepEqual(
    getSundayScheduleDisplayState("sacrament", "2025-02-02", "2025-02-02"),
    { isCurrent: true, hasSubtleFill: false },
  );
  assert.deepEqual(
    getSundayScheduleDisplayState("fast_testimony", "2025-02-09", "2025-02-02"),
    { isCurrent: false, hasSubtleFill: false },
  );
});
