...
test("resolvePersonInput allows member or free text, never both", () => {
  ...
  assert.deepEqual(resolvePersonInput({ memberId: "m1", personName: null }), {
    memberId: "m1",
    personName: null,
  });
  assert.deepEqual(
  ...
  assert.deepEqual(resolvePersonInput({ memberId: "  ", personName: " " }), {
    memberId: null,
    personName: null,
  });
  assert.throws(() =>
...
test("assertTransitionEmpty rejects any data on a transition", () => {
  ...
  assert.throws(() =>
    assertTransitionEmpty("transition", null, null, {
      memberId: "m1",
      personName: null,
    }),
  );
...
test("assertNoAdjacentConductorText checks the final order", () => {
  ...
  const conductor = (id, n, orderIndex = n) =>
    row(
      id,
      "conductor_text",
      "program",
      orderIndex,
      `2025-01-0${n}T00:00:00.000Z`,
    );
  const talk = (id, n, orderIndex = n) =>
