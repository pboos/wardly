...
try {
  ...
  const { verifyBusinessTasks } = await import("./business-tasks.mjs");
  await verifyBusinessTasks(prisma, service, ward, otherWard);
  const { verifySacramentPeople } = await import("./sacrament-people.mjs");
  await verifySacramentPeople(prisma, service, ward, otherWard);
  ...
  const { verifyAddItemPlacement } = await import("./add-item-placement.mjs");
  await verifyAddItemPlacement(service, loadSundayMeeting, ward.id);
  const { verifyCarryForwardText } = await import("./carry-forward-text.mjs");
  await verifyCarryForwardText(service, loadSundayMeeting, ward.id);
